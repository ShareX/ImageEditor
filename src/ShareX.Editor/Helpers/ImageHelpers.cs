...
public static class ImageHelpers
{
    ...
    {
        return new ShareX.Editor.ImageEffects.FilterBorderImageEffect(type, size, dashStyle, color).Apply(source);
    }

    public static SKBitmap ApplyOutline(SKBitmap source, int size, int padding, SKColor color)
    {
        return new ShareX.Editor.ImageEffects.FilterOutlineImageEffect(size, padding, color).Apply(source);
    }
    ...
    {
        return new ShareX.Editor.ImageEffects.FilterShadowImageEffect(opacity, size, darkness, color, offsetX, offsetY, autoResize).Apply(source);
    }

    public static SKBitmap ApplyGlow(SKBitmap source, int size, float strength, SKColor color, int offsetX, int offsetY)
    {
        return new ShareX.Editor.ImageEffects.FilterGlowImageEffect(size, strength, color, offsetX, offsetY).Apply(source);
    }
    ...
    {
        return new ShareX.Editor.ImageEffects.FilterReflectionImageEffect(percentage, maxAlpha, minAlpha, offset, skew, skewSize).Apply(source);
    }
    ...
    {
        return new ShareX.Editor.ImageEffects.FilterTornEdgeImageEffect(depth, range, top, right, bottom, left, curved).Apply(source);
    }

    public static SKBitmap ApplySlice(SKBitmap source, int minHeight, int maxHeight, int minShift, int maxShift)
    {
        return new ShareX.Editor.ImageEffects.FilterSliceImageEffect(minHeight, maxHeight, minShift, maxShift).Apply(source);
    }
}
